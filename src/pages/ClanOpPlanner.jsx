...
const ClanOpPlanner = () => {
    ...
    }, [worldUrl, clanTag, maxSnobDist, targetInput, planList, selectedTarget, selectedDateTime, suggestionMode, selectedPlayer, hiddenTargets, hiddenSources, manualPlayer, manualUnit, bbCols]);
    ...
    const loadWorldClans = async () => {
        ...
        try {
            ...
            allyData.split("\n").forEach(line => {
                ...
                if (p.length >= 3) {
                    clans.push({
                        id: p[0], name: decodeURIComponent(p[1]).replace(/\+/g, ' '), tag: decodeURIComponent(p[2]).replace(/\+/g, ' ')
                    });
                }
            });
            setWorldClans(clans);
    ...
    const handleFetchClan = async () => {
        ...
        try {
            ...
            try {
                ...
                // Regex ile <max_dist> içindeki sayıyı yakalıyoruz
                const distMatch = configData.match(/<max_dist>(\d+)<\/max_dist>/);
                if (distMatch && distMatch[1]) {
                    setMaxSnobDist(parseInt(distMatch[1]));
                }
            } catch (err) {
                console.log("Konfigürasyon çekilemedi, manuel sınır kullanılacak.");
            }
            ...
            storage.set("cop_cache_players", cPlayers);
            ...
            storage.set("cop_cache_allVils", allVils);
            storage.set("cop_cache_cVils", cVils);
